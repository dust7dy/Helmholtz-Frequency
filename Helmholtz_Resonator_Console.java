...
public class Helmholtz_Resonator_Console {
    ...
    public Helmholtz_Resonator_Console() {
        System.out.println(
                ...
                "\nIf you are not sure about what values to insert, you can go to:" +
                "\nhttps://www.youtube.com/channel/UCmxzukyDkrlsZrKW_vwJJGg" +
                "\nand watch the short video on how to use the program.");

        System.out.println("Please insert the volume of the cavity (litres): ");
        Scanner sc = new Scanner(System.in);
        this.V = (float)sc.nextDouble() * 0.001f;

        System.out.println("Please insert the diameter of the neck (cm): ");
        this.r = (float)sc.nextDouble()*0.5f * 0.01f;

        System.out.println("Please insert the type of the resonator (index 0||1): ");
        int i = sc.nextInt();
        sc.nextLine();
        if(i < corr.length) this.c = corr[i];
        else {
            System.out.println(
                    "Faulty input of resonator-index! (Must be either 0 or 1)" +
                    "\nPlease input a valid number and try again.");
        }

        System.out.println("Please insert the length of the neck (cm): ");
        this.lEff = (float)sc.nextDouble()*0.01f + c*r;
